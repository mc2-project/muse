...
where
    ...
    <P::Field as PrimeField>::Params: Fp64Parameters,
    P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
    P::Field: Share<Constant = <P as FixedPointParameters>::Field>,
    P::Field: AuthShare,
{
    fn evaluate_with_method(
        &self,
...
impl<P> LinearLayer<AdditiveShare<FixedPoint<P>>, FixedPoint<P>>
where
    P: FixedPointParameters,
    <P::Field as PrimeField>::Params: Fp64Parameters,
    P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
    ...
    P::Field: AuthShare,
{
    pub fn evaluate_naive_auth(
        &self,
        input: &Input<AuthAdditiveShare<P::Field>>,
        output: &mut Output<AuthAdditiveShare<P::Field>>,
    ) {
        match self {
            LinearLayer::AvgPool { dims: _, params } => params.avg_pool_naive_auth(input, output),
            LinearLayer::Identity { .. } => {
                *output = input.clone();
                let one = FixedPoint::<P>::one().inner;
                for elem in output.iter_mut() {
                    *elem = *elem * one;
                }
            }
...
impl<P: FixedPointParameters> LinearLayerInfo<AdditiveShare<FixedPoint<P>>, FixedPoint<P>>
where
    <P::Field as PrimeField>::Params: Fp64Parameters,
    P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
    P::Field: Share<Constant = <P as FixedPointParameters>::Field>,
    ...
    pub fn evaluate_naive_auth(
        ...
        match self {
            LinearLayerInfo::AvgPool {
                ...
                let params = AvgPoolParams::new(*pool_h, *pool_w, *stride, *normalizer);
                params.avg_pool_naive_auth(input, output)
            }
            LinearLayerInfo::Identity => {
                *output = input.clone();
                let one = FixedPoint::<P>::one().inner;
