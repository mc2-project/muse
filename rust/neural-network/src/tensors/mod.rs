...
use crypto_primitives::{AdditiveShare, AuthAdditiveShare, AuthShare, Share};
use ndarray::Array4;
use num_traits::Zero;
...
impl<T: AuthShare> Input<T> {
    pub fn auth_share_from_parts(share: Input<T>, mac: Input<T>) -> Input<AuthAdditiveShare<T>> {
        assert_eq!(share.dim(), mac.dim());
        let combined = share
            .into_iter()
            .zip(mac.into_iter())
            .map(|(s, m)| AuthAdditiveShare::new(*s, *m))
            .collect();
        Input::from_shape_vec(share.dim(), combined).expect("Shapes should be same")
    }

    pub fn auth_share<R: RngCore + CryptoRng>(
        &self,
        mac_key: &<T as Share>::Ring,
        rng: &mut R,
    ) -> (Input<AuthAdditiveShare<T>>, Input<AuthAdditiveShare<T>>) {
        let mut share_1 = Vec::with_capacity(self.len());
        let mut share_2 = Vec::with_capacity(self.len());
        for inp in self.iter() {
            let (s1, s2) = inp.auth_share(mac_key, rng);
            share_1.push(s1);
            share_2.push(s2);
        }
    ...
    pub fn auth_share_with_randomness(
        &self,
        mac_key: &<T as Share>::Ring,
        r1: &Input<<T as Share>::Ring>,
        r2: &Input<<T as Share>::Ring>,
    ) -> (Input<AuthAdditiveShare<T>>, Input<AuthAdditiveShare<T>>) {
        assert_eq!(r1.dim(), self.dim());
        assert_eq!(r1.dim(), r2.dim());
        let mut share_1 = Vec::with_capacity(r1.len());
        let mut share_2 = Vec::with_capacity(r1.len());
        self.iter()
            .zip(r1.iter())
            .zip(r2.iter())
            .map(|((elem, r1), r2)| T::auth_share_with_randomness(elem, mac_key, r1, r2))
            .for_each(|(s1, s2)| {
                share_1.push(s1);
                share_2.push(s2);
            });
        let s1 = Input::from_shape_vec(r1.dim(), share_1).expect("Shapes should be same");
        let s2 = Input::from_shape_vec(r1.dim(), share_2).expect("Shapes should be same");
        (s1, s2)
    }

    #[inline]
    pub fn unwrap_auth_value(other: Input<AuthAdditiveShare<T>>) -> Self {
        let values = other.iter().map(|e| e.get_value().inner).collect();
        Input::from_shape_vec(other.dim(), values).expect("Shapes should be same")
    }

    #[inline]
    pub fn unwrap_auth_mac(other: Input<AuthAdditiveShare<T>>) -> Self {
        let values = other.iter().map(|e| e.get_mac().inner).collect();
        Input::from_shape_vec(other.dim(), values).expect("Shapes should be same")
    }
}
...
impl<I> Input<I> {
    ...
    /// base field `Fp64` backed by a `u64` (thus allowing lazy reduction),
    ...
    /// If the underlying elements of `Input` are `FixedPoint` elements with
    /// base field F we can convert to Input<F>
    pub fn to_base<P>(&self) -> Input<P::Field>
    where
        P: FixedPointParameters,
        <P::Field as PrimeField>::Params: Fp64Parameters,
        P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
        I: Copy + Into<FixedPoint<P>>,
    {
        let mut input: Input<P::Field> = Input::zeros(self.dim());
        input.iter_mut().zip(self).for_each(|(e1, e2)| {
            let fp: FixedPoint<P> = Into::<FixedPoint<P>>::into(*e2);
            *e1 = fp.inner;
        });
...
where
    F: PrimeField,
    <F as PrimeField>::Params: Fp64Parameters,
    F: PrimeField<BigInt = <<F as PrimeField>::Params as FpParameters>::BigInt>,
{
    /// If the underlying elements of `Input` are elements of a base field
    /// 'Fp64' ` backed by a `u64` we can convert to Input<u64>
    pub fn to_u64(&self) -> Input<u64> {
        let mut input: Input<u64> = Input::zeros(self.dim());
        input.iter_mut().zip(self).for_each(|(e1, e2)| {
            *e1 = e2.into_repr().0;
        });
...
impl<P: FixedPointParameters> From<Input<AuthAdditiveShare<P::Field>>>
    for Input<AdditiveShare<FixedPoint<P>>>
where
    <P::Field as PrimeField>::Params: Fp64Parameters,
    P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
    P::Field: AuthShare,
{
    #[inline]
    fn from(other: Input<AuthAdditiveShare<P::Field>>) -> Self {
        let values = other
            .iter()
            .map(|e| FixedPoint::new(e.get_value().inner).into())
            .collect();
        Input::from_shape_vec(other.dim(), values).expect("Shapes should be same")
    }
}
