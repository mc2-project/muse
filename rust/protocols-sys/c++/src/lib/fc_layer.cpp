...
u64* fc_preprocess_invert(vector<Ciphertext> &r_mac_ct, const Metadata &data, BatchEncoder &batch_encoder, Decryptor &decryptor) {
    // Decrypt
    uv64 result;
    Plaintext tmp;
    decryptor.decrypt(r_mac_ct[0], tmp);
    batch_encoder.decode(tmp, result);

    u64* final_result = new u64[data.image_size];
    for(int col = 0; col < data.image_size; col++){
